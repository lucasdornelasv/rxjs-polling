...
import { JobStatusInfo, JobStatusType, RxPoolingTask } from '../src/pooling-task';

type JobFake = {id: number; error?: boolean; success?: boolean; running?: boolean};
...
describe('RxPoolingTask', () => {
  test('Should Job Success', (done) => {
    const pooling = new RxPoolingTask({
      checkInterval: 50,
      jobFactory: () => {
        return createJob(1000, false)
      },
      checkJobFn: jobId => {
        return checkJob(jobId).pipe(map(res => {
